...
```bash
fluentci gh init -t dotnet_pipeline
```

Or, if you already have a `.fluentci` folder (generated from `fluentci init -t dotnet`) in your project:
  ...
  test:
    ...
    steps:
      ...
      - name: Run Dagger Pipelines
        run: dagger run fluentci dotnet_pipeline
